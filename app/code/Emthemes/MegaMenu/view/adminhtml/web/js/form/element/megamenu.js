...
define([
    ...
    $("#save").click(function(){
        $(this).attr("disabled", true);
        $("#save_and_continue").attr("disabled", true);
    });
    return Abstract.extend({
        ...
        editColumn: function(ui,e){
            var content= $('.show-modal-addClass');
            var form = document.getElementById('addclasscol');
            var classname = form.elements.classname.value;
            content.addClass(classname);
            content.removeClass('show-modal-addClass');
        ...
        gridSystemGenerator: function() {
            $(".lyrow .preview input").bind("keyup", function() {
                ...
                $.each(n, function(n, r) {
                    e = e + parseInt(r);
                    var button = '<button type="button" class="add-col-class btn btn-edit btn-mini" data-toggle="modal" data-target="#addClass" >Add</button>';
                    t += '<div class="span' + r + ' column">' + button + '</div>'
                });
                if (e == 12) {
